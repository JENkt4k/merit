...
- Copy-only `old(...)` snapshots prevent shallow native aliases of owned storage
...
- Semantic rejection of direct and cross-kind recursive by-value aggregates
...
- Typed bootstrap top-level declaration index with brace-depth isolation and independent syntax-span comparison
- Deterministic typed bootstrap diagnostics with interpreter/native/reference parity
...
- Typed postfix call, argument-list, and field-access bootstrap expression records
- Typed direct-constructor and single-type generic-call bootstrap expression records
