...
The next target is `v0.1.0-alpha.2`, focused on replacing the Python-hosted normal compilation path. The Merit-native lexer/source model now covers identifiers/keywords, exact numerics, strings, maximal operators, comments, invalid strings, and byte-stable spans against an independent differential corpus. The first typed parser slice indexes every top-level declaration kind while excluding nested bodies; complete declaration/clause/statement/expression records are next. Unrelated language expansion and hosted CI remain deferred.
