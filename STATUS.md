...
The next target is `v0.1.0-alpha.2`, focused on replacing the Python-hosted normal compilation path. The Merit-native lexer/source model and statement envelopes are established. The first typed expression tree now preserves atoms/groups and enforces multiplicative, additive, and comparison precedence against an independent oracle. Calls, fields, constructors, malformed-operator recovery, and typed statement/clause operands remain before the parser gate can close. Unrelated language expansion and hosted CI remain deferred.
