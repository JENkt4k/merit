...
- Owned vector replacement materializes receiver, index, and replacement once in left-to-right order before dropping the displaced element.
...
- Native transfer materializes destination and source addresses once, in that order, before validating or moving storage.
...
- Value-mode owned parameters are callee-owned: they are implicitly dropped unless explicitly dropped or moved onward, including through return.
...
- Parameters, locals, and match payloads carry deterministic semantic binding IDs. Shadowed source names remain distinct owners across checking, interpreter frames, MIR cleanup, and generated C.
