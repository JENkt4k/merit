from __future__ import annotations

import importlib.util
import json
from pathlib import Path
import re
import shutil
import subprocess

from merit.bootstrap.ast_parity import ast_parity_observations
from merit.bootstrap.parity import build_parity_report, markdown_summary
from merit.bootstrap.repository_corpus import load_repository_corpus
from merit.project.build import build, interpret
from merit.project.loader import load_project


ROOT = Path(__file__).resolve().parents[2]
PROJECT = ROOT / "examples/projects/bootstrap_lexer"
MANIFEST = ROOT / "tests/project/bootstrap_corpus_v1.json"
REFERENCE_PATH = Path(__file__).with_name("test_bootstrap_lexer.py")


def _load_reference_module():
    specification = importlib.util.spec_from_file_location(
        "merit_bootstrap_ast_parity_reference", REFERENCE_PATH
    )
    assert specification is not None and specification.loader is not None
    module = importlib.util.module_from_spec(specification)
    specification.loader.exec_module(module)
    return module


REFERENCE = _load_reference_module()


def _probe_source(expressions: list[str]) -> str:
    calls = []
    for index, expression in enumerate(expressions):
        literal = json.dumps(expression)
        calls.append(
            f"        let source_{index}: Buffer = buffer_from_string(allocator, {literal});\n"
            f"        emit_ast_case(source_{index}, allocator);\n"
            f"        drop(source_{index});"
        )
    body = "\n".join(calls)
    return f'''module bootstrap_ast_parity_probe
import bootstrap_tokens;
import bootstrap_syntax;
import bootstrap_lexer_core;

capability allocate;

fn emit_ast_case(borrow source: Buffer, allocator: Allocator) -> i32
requires_caps [allocate]
{{
    let tokens: Vec<Token> = lex(source, allocator);
    let expressions: Vec<ExpressionNode> = parse_expression_tokens(source, tokens, allocator);
    print(validate_expression_ast_records(expressions));
    let ast_nodes: Vec<AstNodeRecord> = lower_expression_ast_records(expressions, allocator);
    print(vec_len<AstNodeRecord>(ast_nodes));
    var index: i64 = 0;
    while (index < vec_len<AstNodeRecord>(ast_nodes)) {{
        let node: AstNodeRecord = vec_get<AstNodeRecord>(ast_nodes, index);
        print(ast_kind(node));
        print(ast_start(node));
        print(ast_length(node));
        print(ast_left(node));
        print(ast_right(node));
        print(ast_group_start(node));
        print(ast_group_length(node));
        print(ast_group_parent(node));
        index = checked_add(index, 1);
    }}
    drop(ast_nodes);
    drop(expressions);
    drop(tokens);
    return 0;
}}

fn main() -> i32 {{
    with capability allocate {{
        let allocator: Allocator = system_allocator();
{body}
    }}
    return 0;
}}
'''


def _project_with_corpus_probe(tmp_path: Path, expressions: list[str]):
    project_root = tmp_path / "bootstrap_ast_parity"
    shutil.copytree(PROJECT, project_root, ignore=shutil.ignore_patterns("build"))

    # The copied fixture normally owns the project entrypoint. The parity probe
    # becomes the temporary entry while retaining every lexer/parser function.
    lexer_path = project_root / "src/lexer.mrt"
    lexer = lexer_path.read_text(encoding="utf-8")
    lexer, replacements = re.subn(r"\nfn main\(\) -> i32 \{", "\nfn fixture_main() -> i32 {", lexer, count=1)
    assert replacements == 1
    lexer_path.write_text(lexer, encoding="utf-8")

    (project_root / "src/ast_parity_probe.mrt").write_text(
        _probe_source(expressions), encoding="utf-8"
    )
    manifest = project_root / "Merit.toml"
    text = manifest.read_text(encoding="utf-8")
    text = text.replace('entry = "src/lexer.mrt"', 'entry = "src/ast_parity_probe.mrt"')
    manifest.write_text(text, encoding="utf-8")
    return load_project(manifest), project_root


def _parse_probe_output(output: str, case_count: int):
    values = [int(value) for value in output.splitlines()]
    cursor = 0
    cases = []
    for _ in range(case_count):
        assert cursor + 2 <= len(values)
        validation = values[cursor]
        count = values[cursor + 1]
        cursor += 2
        field_count = count * 8
        assert cursor + field_count <= len(values)
        fields = values[cursor : cursor + field_count]
        cursor += field_count
        records = [
            tuple(fields[index : index + 8])
            for index in range(0, len(fields), 8)
        ]
        cases.append((validation, records))
    assert cursor == len(values)
    return cases


def _observations(cases, actual):
    observations = []
    for case, (validation, records) in zip(cases, actual, strict=True):
        assert validation == 0, case.case_id
        reference_records = REFERENCE.reference_expression(case.text)
        observations.extend(
            ast_parity_observations(case.case_id, reference_records, records)
        )
    return observations


def test_repository_expression_corpus_has_real_ast_parity_in_interpreter_and_native(tmp_path):
    corpus = load_repository_corpus(MANIFEST)
    cases = corpus.for_stage("ast")
    assert len(cases) == 12
    expressions = [case.text for case in cases]
    project, project_root = _project_with_corpus_probe(tmp_path, expressions)

    interpreted = _parse_probe_output(interpret(project), len(cases))
    interpreted_report = build_parity_report(
        corpus, _observations(cases, interpreted), stages=["ast"]
    )
    assert interpreted_report.complete, markdown_summary(interpreted_report)
    assert interpreted_report.stage_counts() == {"ast": (12, 12)}

    _, _, executable = build(project, project_root / "ast_parity")
    native_output = subprocess.run(
        [str(executable)], check=True, text=True, capture_output=True
    ).stdout
    native = _parse_probe_output(native_output, len(cases))
    native_report = build_parity_report(
        corpus, _observations(cases, native), stages=["ast"]
    )
    assert native_report.complete, markdown_summary(native_report)
    assert native_report.stage_counts() == {"ast": (12, 12)}

    # Interpreter/native agreement is an independent requirement from both
    # implementations matching the Python contract oracle.
    assert native == interpreted
