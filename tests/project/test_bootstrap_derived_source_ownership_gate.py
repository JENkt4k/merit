...
def _probe() -> str:
    ...
    resource = SOURCE.index("Resource")
    i64 = SOURCE.index("i64", SOURCE.index("let x"))
...
fn main()->i32 {{
 with capability allocate {{
  ...
  var type_ownership:Vec<SourceTypeOwnershipEntry>=vec_new<SourceTypeOwnershipEntry>(allocator,2);
  vec_push<SourceTypeOwnershipEntry>(type_ownership,source_type_ownership_entry({resource},8,1));
  vec_push<SourceTypeOwnershipEntry>(type_ownership,source_type_ownership_entry({i64},3,0));
  ...
  print(lower_resolved_source_function_assembly_from_source_types(
    source,tokens,statements,operands,variants,-1,capabilities,type_ownership,allocator,
  ...
  drop(contracts); drop(metadata); drop(body); drop(bindings); drop(type_ownership);
