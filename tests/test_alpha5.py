...
def test_new_project_builds_and_runs(tmp_path):
    ...
    subprocess.run([*MERIT_COMMAND, "new", str(project)], check=True)
    source = project / "src" / "main.mrt"
    assert source.exists()
    completed = subprocess.run(
        [*MERIT_COMMAND, "exec", str(source)],
        check=True,
        text=True,
        capture_output=True,
    )
