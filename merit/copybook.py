            ...
            "fields": [
                {
                    "level": f.level,
                    "name": f.name,
                    "picture": asdict(f.picture),
                    "usage": f.usage,
                    "occurs": f.occurs,
                    "offset": f.offset,
                    "element_length": f.length,
                    "storage_length": f.length * f.occurs,
...
_USAGE = re.compile(
    r"\b(?:USAGE\s+(?:IS\s+)?)?(COMP-3|PACKED-DECIMAL|COMP-5|COMP-4|COMP|BINARY|DISPLAY)\b",
    re.IGNORECASE,
)
...
def _logical_lines(source: str) -> list[tuple[int, str]]:
    ...
    for line_no, raw in enumerate(source.splitlines(), 1):
        line = raw.rstrip("\r\n")
        # Traditional fixed-format copybooks use cols 1-6 for sequence and col 7
        # for comment/continuation. Accept that shape without requiring it.
...
def _count_picture_atoms(fragment: str, atom: str) -> int:
    ...
    while pos < len(fragment):
        match = pattern.match(fragment, pos)
        if not match:
...
def parse_picture(raw: str) -> Picture:
    ...
    return Picture(
        raw=raw.upper(), kind="numeric", digits=integer_digits + scale,
        scale=scale, signed=signed,
    )


def _storage_length(pic: Picture, usage: str) -> int:
    if pic.kind == "text":
        if usage != "DISPLAY":
...
def parse_copybook(source: str, *, name: str = "COPYBOOK") -> CopybookSchema:
    ...
    for line_no, statement in _logical_lines(source):
        ...
        pic = parse_picture(pic_raw)
        length = _storage_length(pic, usage)
        fields.append(CopybookField(
            level=level, name=field_name, picture=pic, usage=usage, occurs=occurs,
            offset=offset, length=length, source_line=line_no,
        ))
...
def decode_zoned(data: bytes, *, scale: int = 0, signed: bool = False) -> Decimal:
    ...
    for index, byte in enumerate(data):
        ...
        digits.append(digit)
    unscaled = int("".join(str(d) for d in digits)) * sign
    return Decimal(unscaled).scaleb(-scale)


def encode_zoned(value: Decimal | int | str, *, digits: int, scale: int = 0, signed: bool = False) -> bytes:
    ...
    out = bytearray(0xF0 | int(ch) for ch in text)
    if signed:
        out[-1] = (0xD0 if negative else 0xC0) | int(text[-1])
    return bytes(out)


def decode_comp3(data: bytes, *, digits: int, scale: int = 0, signed: bool = True) -> Decimal:
    nibbles: list[int] = []
    for byte in data:
    ...
    # Even digit counts have one leading pad nibble.
    while len(nibbles) > digits:
        pad = nibbles.pop(0)
        if pad != 0:
            raise CopybookError("non-zero COMP-3 pad nibble")
    if len(nibbles) != digits or any(n > 9 for n in nibbles):
        raise CopybookError("invalid COMP-3 digit payload")
    unscaled = int("".join(str(n) for n in nibbles) or "0") * sign
    return Decimal(unscaled).scaleb(-scale)


def encode_comp3(value: Decimal | int | str, *, digits: int, scale: int = 0, signed: bool = True) -> bytes:
    decimal = Decimal(value)
    quantum = Decimal(1).scaleb(-scale)
    if decimal.quantize(quantum) != decimal:
...
def encode_binary(value: Decimal | int | str, *, length: int, scale: int = 0, signed: bool = False) -> bytes:
    decimal = Decimal(value)
    quantum = Decimal(1).scaleb(-scale)
    if decimal.quantize(quantum) != decimal:
...
def _decode_element(field: CopybookField, data: bytes, *, codec: str) -> Any:
    ...
    if field.usage == "DISPLAY":
        return decode_zoned(data, scale=pic.scale, signed=pic.signed)
    if field.usage == "COMP-3":
        return decode_comp3(data, digits=pic.digits, scale=pic.scale, signed=pic.signed)
    if field.usage == "BINARY":
        return decode_binary(data, scale=pic.scale, signed=pic.signed)
...
def _encode_element(field: CopybookField, value: Any, *, codec: str) -> bytes:
    ...
    if pic.kind == "text":
        text = str(value)
        encoded = encode_ebcdic(text, codec=codec)
    ...
    if field.usage == "DISPLAY":
        return encode_zoned(value, digits=pic.digits, scale=pic.scale, signed=pic.signed)
    if field.usage == "COMP-3":
        return encode_comp3(value, digits=pic.digits, scale=pic.scale, signed=pic.signed)
    if field.usage == "BINARY":
        return encode_binary(value, length=field.length, scale=pic.scale, signed=pic.signed)
    raise CopybookError(f"unsupported usage {field.usage}")


def decode_record(schema: CopybookSchema, data: bytes, *, codec: str = "cp037") -> dict[str, Any]:
    if len(data) != schema.record_length:
        raise CopybookError(f"record length {len(data)} != schema length {schema.record_length}")
    result: dict[str, Any] = {}
    for field in schema.fields:
        values = []
        for occurrence in range(field.occurs):
            start = field.offset + occurrence * field.length
            values.append(_decode_element(field, data[start:start + field.length], codec=codec))
        result[field.name] = values if field.occurs != 1 else values[0]
    return result


def encode_record(schema: CopybookSchema, values: Mapping[str, Any], *, codec: str = "cp037") -> bytes:
    output = bytearray(schema.record_length)
    expected = {field.name for field in schema.fields}
    missing = expected - set(values)
    extra = set(values) - expected
    if missing or extra:
        raise CopybookError(f"record keys mismatch; missing={sorted(missing)}, extra={sorted(extra)}")
    for field in schema.fields:
...
def generate_merit(schema: CopybookSchema, *, module: str = "copybook_generated") -> str:
    """Generate a stable canonical Merit representation, never a raw-memory alias."""
    lines = [f"module {_ident(module)}", ""]
    text_lengths = sorted({f.picture.chars for f in schema.fields if f.picture.kind == "text"})
    for length in text_lengths:
        lines.append(f'stable("copybook-text-{length}-v1") struct CopybookText{length} {{')
    ...
    for field in schema.fields:
        pic = field.picture
        if pic.kind == "numeric" and pic.scale:
            key = (pic.digits, pic.scale)
            if key not in decimal_types:
                type_name = f"CopybookDecimal{pic.digits}S{pic.scale}"
                decimal_types[key] = type_name
                lines.extend([f"decimal {type_name}({pic.digits}, {pic.scale}, half_even);", ""])
    struct_name = _type_name(schema.name)
    lines.append(f'pub stable("copybook-{_ident(schema.name)}-v1") struct {struct_name} {{')
    for field in schema.fields:
        pic = field.picture
        if field.occurs != 1:
            # Stable v1 representation keeps fixed OCCURS elements explicit.
            for occurrence in range(field.occurs):
                if pic.kind == "text":
                    merit_type = f"CopybookText{pic.chars}"
                elif pic.scale:
                    merit_type = decimal_types[(pic.digits, pic.scale)]
                else:
                    merit_type = "i64" if pic.signed else "u64"
...
def verify_golden(schema: CopybookSchema, vectors: Iterable[Mapping[str, Any]], *, codec: str = "cp037") -> None:
    for index, vector in enumerate(vectors):
        ...
        if encoded.hex() != expected_hex:
            raise CopybookError(f"golden {index}: encoded {encoded.hex()} != {expected_hex}")
        ...
        if normalized != expected:
            raise CopybookError(f"golden {index}: decoded {normalized!r} != {expected!r}")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="merit-copybook", description="Deterministic COBOL copybook migration toolkit")
    sub = parser.add_subparsers(dest="command", required=True)
    inspect = sub.add_parser("inspect", help="parse a copybook and emit its canonical manifest")
    inspect.add_argument("copybook", type=Path)
    generate = sub.add_parser("generate", help="generate stable canonical Merit declarations")
    generate.add_argument("copybook", type=Path)
    generate.add_argument("-m", "--module", default="copybook_generated")
    verify = sub.add_parser("verify", help="verify JSON golden vectors byte-for-byte and round-trip")
    ...
    try:
        ...
        if args.command == "inspect":
            ...
            print(f"verified {len(payload)} golden vector(s); record_length={schema.record_length}")
        return 0
